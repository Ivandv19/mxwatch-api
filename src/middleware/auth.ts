...
export const authMiddleware = async (c: Context, next: Next) => {
  ...
  // -----------------------------------------------------------------------------
  if (!expectedKey) {
    console.error('❌ CONFIGURATION ERROR: API_KEY is missing from environment variables.');
    return c.json(
      {
        success: false,
        error: 'Internal Server Error',
        message: 'Acceso bloqueado: El servidor no ha sido configurado con una clave de seguridad.',
      },
      500
    );
  }
  ...
  if (apiKey !== expectedKey) {
    ...
    console.error(`🔴 UNAUTHORIZED ACCESS ATTEMPT: Denied request from ${userAgent}`);

    return c.json(
      {
        success: false,
        error: 'Unauthorized',
        message: 'Acceso denegado. Se requiere una API Key válida en el header "x-api-key".',
      },
      401
    );
  }
